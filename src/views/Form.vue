<template>
  <v-form>
    <v-container>
      <div class="header">
        <div>
          <v-banner color="blue" lines="two" max-width="800"
            ><v-icon slot="icon" color="black" size="48"
              >mdi-package-variant</v-icon
            >
            <v-banner-text class="text-left">
              <h2>Pendataan Bantuan Sosial Kemensos</h2>
            </v-banner-text>
          </v-banner>
        </div>
      </div>
      ...
      >
        ...
        >
          ...
          <v-text-field
            ...
            @keypress="cekNumber"
            :rules="[
              (v) => !!v || 'Nomor Kartu Keluarga wajib diisi',
              rules.kk,
            ]"
          ...
          <div class="row mb-4">
            ...
            <div class="col-7 col-md-9">
              ...
              <div>
                <div
                  class="text-caption text--secondary font-weight-light font-italic d-flex"
                  v-for="(message, index) in messagesFoto"
                  :key="`message-${index}`"
                >
                  <div>
          ...
          <div class="row mb-4">
            ...
            <v-select
              class="col-6"
              :disabled="kotadata.length == 0"
              v-model="kota"
              @change="onchangeKota"
              :items="kotadata"
              item-text="name"
              item-value="id"
              :rules="[(v) => !!v || 'Kota/Kab. wajib diisi']"
          ...
          <div class="row mb-4">
            <v-select
              ...
              item-text="name"
              item-value="id"
              :rules="[(v) => !!v || 'Kecamatan wajib diisi']"
            ...
            <v-select
              ...
              item-text="name"
              item-value="id"
              :rules="[(v) => !!v || 'Kelurahan/Desa wajib diisi']"
          ...
          <div class="row mb-4 ">
            <v-text-field
              ...
              counter
              label="Penghasilan Sebelum Pandemi (Rupiah)"
              color="accent"
              type="number"
              required
            ></v-text-field>
            ...
            <v-text-field
              ...
              counter
              label="Penghasilan Setelah Pandemi (Rupiah)"
              color="accent"
              type="number"
              required
            ></v-text-field>
...
export default {
  data: () => ({
    ...
    messagesFoto: [
      ...
      "Format file PNG/JPG/JPEG/BMP",
    ],
    rules: {
      photoSize: (v) => !v || v.size <= 2000000 || "Ukuran file terlalu besar",
      photoType: (v) =>
        !v ||
        ["image/png", "image/jpeg", "image/jpeg", "image/bmp"].indexOf(
          v.type
        ) >= 0 ||
        "Format file salah",
      ...
      alamat: (v) =>
        (v && v.length <= 255) || "Alamat tidak boleh lebih dari 255 karakter",
    },
    listCountry: [],
    listState: [],
    listCities: [],
  ...
  methods: {
    async getData() {
      await fetch(
        `https://ekiditapermana.github.io/api-wilayah-indonesia/api/provinces.json`
      )
    ...
    async onchangeProvinsi() {
      await fetch(
        `https://ekiditapermana.github.io/api-wilayah-indonesia/api/regencies/${this.provinsi}.json`
      )
        .then((response) => response.json())
        .then((regencies) => {
          console.log(regencies);
          this.kotadata = regencies;
          this.kota = 0;
          this.kecamatan = 0;
          this.desa = 0;
        });
    ...
    async onchangeKota() {
      await fetch(
        `https://ekiditapermana.github.io/api-wilayah-indonesia/api/districts/${this.kota}.json`
      )
        .then((response) => response.json())
        .then((districts) => {
          console.log(districts);
          this.kecamatandata = districts;
          this.kecamatan = 0;
          this.desa = 0;
        });
    },

    async onchangeKecamatan() {
      await fetch(
        `https://ekiditapermana.github.io/api-wilayah-indonesia/api/villages/${this.kecamatan}.json`
      )
        .then((response) => response.json())
        .then((villages) => {
          console.log(villages);
          this.desadata = villages;
          this.desa = 0;
        });
    },
